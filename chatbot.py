...
template = """
        ...
        **Instructions** :
        ...
        - Si vous ne trouvez pas l'information, répondez uniquement par "Je ne sais pas." sans ajouter d'autres détails.
...
class ModelFactory:
    @staticmethod
    def create_vector_store(data: pd.DataFrame, 
    ...
    @staticmethod
    def create_llama(model_path: str = CHAT_MODEL,
                     n_gpu_layers: int = DEFAULT_N_GPU_LAYERS,
                     n_batch: int = DEFAULT_N_BATCH,
                     temperature: float = DEFAULT_TEMPERATURE,
                     max_tokens: int = DEFAULT_MAX_TOKENS,
                     seed: int = DEFAULT_SEED,
                     t_top: float = DEFAULT_T_TOP,
                     streaming: bool = DEFAULT_STREAMING) -> LlamaCpp:
        """Créer et retourner un modèle Llama."""
...
def main():
    ...
    llm = ModelFactory.create_llama()
    vector_store = ModelFactory.create_vector_store(data, batch_size=args.batch_size)
